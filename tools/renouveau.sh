...
echo -e "";
echo "{ \"response\": [ { \"datetime\": $(grep ro\.build\.date\.utc $OUT/system/build.prop | cut -d= -f2), \"filename\": \"$(basename $(ls $OUT/Renouveau*.zip))\", \"id\": \"$((sha256sum $OUT/Renouveau*.zip) | cut -d ' ' -f1)\", \"romtype\": \"$RENOUVEAU_BUILDTYPE\", \"size\": $(stat -c%s $OUT/Renouveau*.zip), \"url\": \"https://github.com/RenouveauOS/android_device_$(grep ro\.product\.manufacturer $OUT/system/build.prop | cut -d= -f2)_$RENOUVEAU_BUILD/download/v$(grep ro\.renouveau\.build\.version $OUT/system/build.prop | cut -d= -f2)-$(grep renouveau\.ota\.version $OUT/system/build.prop | cut -d= -f2)-$RENOUVEAU_BUILDTYPE/$(grep ro\.renouveau\.version $OUT/system/build.prop | cut -d= -f2)\", \"version\": \"$(grep ro\.renouveau\.build\.version $OUT/system/build.prop | cut -d= -f2)\"  }]}" | tee $OUT/$RENOUVEAU_BUILD.json
