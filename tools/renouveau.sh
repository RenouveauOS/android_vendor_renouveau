...
echo -e "";
echo "{ \"response\": [ { \"datetime\": $(grep ro\.build\.date\.utc $OUT/system/build.prop | cut -d= -f2), \"filename\": \"$(basename $(ls $OUT/Renouveau*.zip))\", \"id\": \"$((sha256sum $OUT/Renouveau-P*.zip) | cut -d ' ' -f1)\", \"romtype\": \"$RR_BUILDTYPE\", \"size\": $(stat -c%s $OUT/Renouveau*.zip), \"url\": \"https://github.com/RenouveauOS/releases/download/$(basename $(ls $OUT/Renouveau*.zip))/$(ls $OUT/Renouveau*.zip)\", \"version\": \"$(grep -m1 ro\.rr\.build\.version $OUT/system/build.prop | cut -d= -f2)\"  }]}" | tee $OUT/$RR_BUILD.json
